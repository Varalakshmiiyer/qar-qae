...
if __name__ == "__main__":
	...
	parser.add_argument('--model_names', type=str, default='avg,qcact,qc,bow')
	...
	results_dir = os.path.join(base_dir, "results/c2vqa-edit-verbs-results/")
	...
	vocab, answers, embedding_matrix, word_to_vector, question_seq_length, caption_seq_length = load_word_data(questions_df, image_captions, exclude_word_list)
	...
	def create_model_and_data(model_name, image_files):
		if model_name == 'image':
			...
			x, y, df = extract_attention_word_caption_features(image_files, image_filenames, image_captions, questions_df, word_to_vector, answers, vocab, max_seq_length, max_seq_length)
			model = attention_question_caption_model(embedding_matrix, len(vocab), embedding_dim, max_seq_length, dropout_rate, len(answers))
	...
	for trial in range(0,trials):
		...
		for model_name in model_names:
			...
			if model_name == "qcatt":
				...
				stats['accuracy'] = scores_map['reason_output_acc']
				stats['relevance_accuracy'] = scores_map['relevance_output_acc']
			elif model_name in ["qcact", "imageact"]:
				scores = model.evaluate(x_test, y_test, batch_size=batch_size)
				scores_map = dict(zip(model.metrics_names, scores))

				stats['accuracy_keras'] = scores_map['reason_output_acc']
